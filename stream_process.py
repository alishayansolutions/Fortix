...
# Initialize the YOLO model and background subtractor
try:
...
def get_fgbg_instance(request_key):
    try:
        ...
        raise RuntimeError(f"Error creating background subtractor: {e}")
...
def process_subsequent_frame(frame, stream_id):
    try:
        ...
        print(f"Error in process_subsequent_frame: {e}")
...
def process_stream_via_model(config , process, streaming_type="RSTP"):
    try:
        ...
        print(f"Error in process_stream_via_model: {e}")    
